...
if ( ! class_exists( 'WC_Multibanco_IfThen_Webdados' ) ) {
	...
	class WC_Multibanco_IfThen_Webdados extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->method_description = __( 'Easy and simple payment using “Payment of Services” at any “Multibanco” ATM terminal or your home banking service. (Only available to customers of Portuguese banks - Payment service provided by ifthenpay)', 'multibanco-ifthen-software-gateway-for-woocommerce' );
							...
							<table class="form-table">
								...
								<tr valign="top">
									...
									<td class="forminp callbackurl">
										<?php echo esc_html( WC_IfthenPay_Webdados()->multibanco_notify_url ); ?>
									</td>
								</tr>
							</table>
			...
			if (
				...
				if ( $arguments_ok ) { // Isto deve ser separado em vários IFs para melhor se identificar o erro
					...
					if ( $orders_exist ) {
						...
						header( 'HTTP/1.1 200 OK' );
						$err = WC_IfthenPay_Webdados()->callback_helper_order_not_found_error( $this->id, $args, 'Multibanco' );
						$this->debug_log( '-- ' . $err, 'warning', true, 'Callback (' . $server_http_host . ' ' . $server_request_uri . ') from ' . $server_remote_addr );
						echo esc_html( $err );
						do_action( 'multibanco_ifthen_callback_payment_failed', 0, $err, $_GET ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					}
