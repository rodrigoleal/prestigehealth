...
if ( ! class_exists( 'WC_MBWAY_IfThen_Webdados' ) ) {
	...
	class WC_MBWAY_IfThen_Webdados extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->allow_international       = ( $this->get_option( 'allow_international' ) === 'yes' ? true : false );
		...
		public function init_form_fields() {
			...
			$this->form_fields = array_merge(
				...
				array(
					...
					'allow_international' => array(
						'title'   => __( 'Allow international phone numbers', 'multibanco-ifthen-software-gateway-for-woocommerce' ),
						'type'    => 'checkbox',
						'label'   => __( 'Show country selector to allow international MB WAY phone numbers', 'multibanco-ifthen-software-gateway-for-woocommerce' ),
						'default' => 'yes',
					),
					'only_above'          => array(
		...
		public function webservice_set_pedido( $order_id, $phone ) {
			return WC_IfthenPay_Webdados()->mbway_webservice_set_pedido( $order_id, $phone, true );
		}
		...
		public function process_payment( $order_id ) {
			...
			if ( $order->get_total() > 0 ) {
				...
				if ( $this->allow_international ) {
					$country_code = isset( $_POST[ $this->id . '_country_code' ] ) ? trim( sanitize_text_field( wp_unslash( $_POST[ $this->id . '_country_code' ] ) ) ) : '';
					if ( empty( $country_code ) ) { // Ticket: https://wordpress.org/support/topic/erro-pagamentos/
						$country_code = isset( $_REQUEST[ $this->id . '_country_code' ] ) ? trim( sanitize_text_field( wp_unslash( $_REQUEST[ $this->id . '_country_code' ] ) ) ) : '';
					}
						...
						<span id="<?php echo esc_attr( $this->id ); ?>_phone_field_container_country_code">
							<select name="<?php echo esc_attr( $this->id ); ?>_country_code" id="<?php echo esc_attr( $this->id ); ?>_country_code">
								...
								foreach ( $countries as $country_code => $country ) {
									?>
									<option value="<?php echo esc_attr( $country_code ); ?>" <?php selected( $default_code, $country_code ); ?>>
										<?php echo esc_html( $country['display'] ); ?>
									</option>
									<?php
								}
								?>
			...
			<script type="text/javascript">
				jQuery( document ).ready( function( $ ) {
					if ( typeof $.fn.selectWoo === 'function' ) {
						$( '#<?php echo esc_js( $this->id ); ?>_country_code' ).selectWoo().trigger( 'change' ).prop( 'required', true );
					}
				} );
			</script>
			<style type="text/css">
		...
		public function validate_fields() {
			...
			if ( empty( $phone ) ) {
				...
				&&
				(
					( $this->allow_international && $country_code !== 'PT' )
					||
					(
						$country_code === 'PT'
						&&
		...
		public function callback() {
			...
			$server_request_uri = str_replace( 'chave=' . trim( $this->secret_key ), 'chave=[CORRECT_ANTIPHISHING_KEY]', $server_request_uri );
				...
				if ( $arguments_ok ) { // Isto deve ser separado em vários IFs para melhor se identificar o erro
					...
					if ( trim( $estado ) === 'PAGO' ) {
						...
						if ( $orders_exist ) {
							...
							header( 'HTTP/1.1 200 OK' );
							$err = WC_IfthenPay_Webdados()->callback_helper_order_not_found_error( $this->id, $args, 'MB WAY' );
